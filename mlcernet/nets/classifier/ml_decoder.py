...
class MLDecClassifier(Classifier):
    def __init__(self, cfg):
        super(MLDecClassifier, self).__init__(**cfg)
        num_of_groups=-1
        decoder_embedding=768
        initial_num_features=self.embed_dim
        zsl=0
        
        embed_len_decoder = 100 if num_of_groups < 0 else num_of_groups
        if embed_len_decoder > self.num_classes-1:
            embed_len_decoder = self.num_classes-1
        ...
        if self.zsl:
            ...
            self.decoder.num_classes = self.num_classes-1
            self.decoder.duplicate_factor = int(self.num_classes-1 / embed_len_decoder + 0.999)
            ...
            self.decoder.duplicate_pooling_bias = torch.nn.Parameter(torch.Tensor(self.num_classes-1))
    ...
    def calc_loss(self,feature_emb, databatch):
        positive_logits = self.calc_logits(feature_emb)  # (bs, num_cls-1)
